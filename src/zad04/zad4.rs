...
fn main() {
  ...
  // 1.
  let res1: usize = lines.clone().map(|line| {
  ...
  let res2: usize = lines.map(|line| {
    ...
    if (left_range.start >= right_range.start && left_range.start <= right_range.end) || (left_range.end >= right_range.start && left_range.end <= right_range.end) {
      return 1;
    }

    if (right_range.start >= left_range.start && right_range.start <= left_range.end) || (right_range.end >= left_range.start && right_range.end <= left_range.end) {
      return 1;
    }
